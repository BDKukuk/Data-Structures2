...
public class MazeSolverClass
   {
    ...
    public MazeSolverClass( int rowHeight, int colWidth,
       ...
       {
        // TODO: Create this initialization constructor for the class
    ...
    public boolean solveMaze()
       {
           LocationData Location = new LocationData(getInitialDirection()
                   ,getStartY(),getStartX());


           setFirstPosition(Location,TRACK_MARKER);

           if(isWallAhead(Location) == false)
           {
               switch(Location.direction)
               {
                   case NORTH:
                       Location = moveToNextCell(new LocationData(Location.direction,startY-HALF_CELL_HEIGHT,startX));
                       diagnosticDump();

                   case EAST:
                       Location = moveToNextCell(new LocationData(Location.direction,startY,startX+HALF_CELL_WIDTH));
                       diagnosticDump();
                   case SOUTH:
                       Location = moveToNextCell(new LocationData(Location.direction,startY+HALF_CELL_HEIGHT,startX));
                       diagnosticDump();
           ...
           else
           {
               if(solveMaze() == false)
               {
                   return solveMazeHelper(Location);
               }
           }
    ...
    public boolean solveMazeHelper( LocationData currentLoc )
       {
           if(isExitAhead(currentLoc) == true)
           {
               moveToExit(currentLoc);
               diagnosticDump();
               return true;
           }
           ...
           if(isWallAhead(currentLoc) == true)
           {
           ...
           if(isWallAhead(currentLoc) == false)
           {
               solveMazeHelper(undoMoveToCell(currentLoc));
               diagnosticDump();
               return true;
           }

           if(solveMazeHelper(currentLoc) == false)
           {
               solveMazeHelper(currentLoc);
               diagnosticDump();
           }
